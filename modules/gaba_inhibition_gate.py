...
class GateParams:
    ...
    vol_norm: float = 0.2             # Realized volatility normalization constant
    vol_clip_max: float = 4.0         # Cap for realized volatility normalization
    vix_drive_weight: float = 0.6     # Mix between VIX and realized volatility threat
    ...
    plasticity_max_delta: float = 0.05  # Clamp for per-step plasticity updates
    ...
    def validate(self) -> None:
        ...
        if self.vol_norm <= 0:
            raise ValueError("vol_norm must be positive")
        if self.vol_clip_max <= 0:
            raise ValueError("vol_clip_max must be positive")
        if not 0.0 <= self.vix_drive_weight <= 1.0:
            raise ValueError("vix_drive_weight must be within [0, 1]")
        ...
        if self.plasticity_max_delta < 0:
            raise ValueError("plasticity_max_delta must be non-negative")
...
class GABAInhibitionGate(nn.Module):
    ...
    def _norm_vix(self, vix: torch.Tensor) -> torch.Tensor:
        """Normalize VIX-like inputs to a bounded threat indicator."""
        return torch.clamp(vix / self.p.vix_norm, 0.0, self.p.vix_clip_max)

    def _norm_realized_vol(self, vol: torch.Tensor) -> torch.Tensor:
        """Normalize realized volatility magnitudes to the threat range."""
        return torch.clamp(vol.abs() / self.p.vol_norm, 0.0, self.p.vol_clip_max)
    ...
    def forward(
        ...
        # 1) GABA release ~ blended threat proxy (implied + realized volatility)
        norm_vix = self._norm_vix(vix)
        norm_realized = self._norm_realized_vol(vol)
        threat = (
            self.p.vix_drive_weight * norm_vix
            + (1.0 - self.p.vix_drive_weight) * norm_realized
        )
        drive = torch.clamp(
            self.p.gaba_drive_scale * threat,
            0.0,
        ...
        if self.p.plasticity_max_delta == 0:
            dw = torch.zeros_like(dw)
        else:
            dw = torch.clamp(
                dw,
                -self.p.plasticity_max_delta,
                self.p.plasticity_max_delta,
            )
        ...
        return gated, GateMetrics(
            ...
            plasticity_delta=float(dw.item()),
        )
