...
## Практичні сценарії інтеграції

Найчастіші сценарії застосування модулів у ланцюжку прийняття рішень:

1. **Режим ринку → ризик → позиція**: `MarketRegimeAnalyzer` визначає режим, `AdaptiveRiskManager`
   переглядає ліміти, `DynamicPositionSizer` рахує рекомендований розмір позиції.
2. **Перевірка якості даних → оптимізація портфеля**: `DataQualityMonitor` відсіює аномальні дані,
   `PortfolioOptimizer` готує ваги для ребалансу.
3. **Планування та виконання стратегій**: `StrategyScheduler` запускає стратегії,
   `ExecutionAnalyzer` контролює якість виконання.

Мінімальні зв’язки між модулями:
...
regime = MarketRegimeAnalyzer(regime_window=100)
risk = AdaptiveRiskManager(base_capital=500_000, risk_tolerance=0.015)
sizer = DynamicPositionSizer(base_capital=500_000, volatility_target=0.12)
