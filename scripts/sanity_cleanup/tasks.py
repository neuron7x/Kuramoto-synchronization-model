...
CONFIG_EXTENSIONS = {".yml", ".yaml", ".ini", ".cfg", ".conf", ".toml", ".json", ".env", ".properties"}
...
def clean_temp_files(context: TaskContext) -> TaskReport:
    ...
    return TaskReport(name="clean_temp_files", status=status, summary=summary, details=tuple(details))
...
def consolidate_scripts(context: TaskContext) -> TaskReport:
    ...
    if discovered and not context.options.dry_run:
        ...
        manifest_path.write_text(
            json.dumps(sorted(discovered, key=lambda item: item["path"]), indent=2, ensure_ascii=False)
            + "\n",
            encoding="utf-8",
        )
...
def standardize_build_targets(context: TaskContext) -> TaskReport:
    ...
    return TaskReport(
        ...
        details=tuple(targets[:10] + ([f"… truncated list, {len(targets) - 10} additional targets"] if len(targets) > 10 else [])),
...
def check_links(context: TaskContext) -> TaskReport:
    ...
    summary = "All checked documentation links resolved" if not broken else f"Detected {len(broken)} broken links"
    details = tuple(broken[:20] + ([f"… truncated list, {len(broken) - 20} additional links"] if len(broken) > 20 else []))
...
def validate_templates(context: TaskContext) -> TaskReport:
    ...
    summary = "Contribution templates validated" if not details else "Contribution templates require attention"
    return TaskReport(name="validate_templates", status=status, summary=summary, details=tuple(details))
...
def collect_package_metadata(context: TaskContext) -> TaskReport:
    ...
    if package_json.exists():
        ...
        metadata["package.json"] = {
            ...
            "dependencies": len((data.get("dependencies") or {})) + len((data.get("devDependencies") or {})),
        }
...
def inventory_configurations(context: TaskContext) -> TaskReport:
    ...
    if inventory and not context.options.dry_run:
        ...
        output_path.write_text(
            json.dumps(sorted(inventory, key=lambda item: item["path"]), indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
...
def find_duplicate_files(context: TaskContext) -> TaskReport:
    ...
    if duplicates and not context.options.dry_run:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(json.dumps(duplicates, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
def check_permissions(context: TaskContext) -> TaskReport:
    ...
    summary = "Permissions look sane" if not flagged else f"Found {len(flagged)} world-writable paths"
    details = tuple(flagged[:20] + ([f"… truncated list, {len(flagged) - 20} additional entries"] if len(flagged) > 20 else []))
...
def directory_inventory(context: TaskContext) -> TaskReport:
    ...
    for path in context.root.iterdir():
        if not path.is_dir() or path.name.startswith(".") or path.name in {"archives", "__pycache__"}:
            continue
        file_count = 0
        dir_count = 0
        total_size = 0
...
def archive_legacy_content(context: TaskContext) -> TaskReport:
    ...
    if context.options.archive_legacy and not context.options.dry_run:
        ...
        for candidate in candidates:
            ...
            archive_name = f"{slug}-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
    ...
    summary = (
        "Archived legacy directories" if details and context.options.archive_legacy else f"Identified {len(candidates)} legacy directories"
    )
    status = TaskStatus.SUCCESS
    return TaskReport(
        ...
        status=status,
        summary=summary,
        details=tuple(details[:10] + ([f"… truncated list, {len(details) - 10} additional entries"] if len(details) > 10 else [])),
