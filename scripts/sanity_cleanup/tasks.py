...
def consolidate_scripts(context: TaskContext) -> TaskReport:
    ...
    if discovered and not context.options.dry_run:
        ...
        manifest_path.write_text(
            json.dumps(
                sorted(discovered, key=lambda item: item["path"]), indent=2, ensure_ascii=False
            )
            + "\n",
            encoding="utf-8",
        )
...
def validate_templates(context: TaskContext) -> TaskReport:
    ...
    return TaskReport(
        name="validate_templates", status=status, summary=summary, details=tuple(details)
    )
...
def inventory_configurations(context: TaskContext) -> TaskReport:
    ...
    if inventory and not context.options.dry_run:
        ...
        output_path.write_text(
            json.dumps(
                sorted(inventory, key=lambda item: item["path"]), indent=2, ensure_ascii=False
            )
            + "\n",
            encoding="utf-8",
        )
