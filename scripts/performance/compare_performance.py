...
_BENCHMARK_LABELS = {
    "tests/performance/test_indicator_benchmarks.py::test_compute_phase_hot_path": "kuramoto.compute_phase[128k]",  # noqa: E501
    "tests/performance/test_indicator_benchmarks.py::test_kuramoto_order_matrix": "kuramoto.order[4096x12]",  # noqa: E501
    "tests/performance/test_indicator_benchmarks.py::test_hierarchical_feature_stack": "hierarchical.features[3x2048]",  # noqa: E501
}
...
def _build_markdown(
    ...
    for item in benchmarks:
        ...
        lines.append(
            f"| {item.name} | {baseline_ms:.3f} | {current_ms:.3f} | {delta} | {ops_delta} | {status_icon} |"  # noqa: E501
        )
    ...
    for item in resources:
        ...
        status_icon = "✅" if item.status == "pass" else "❌"
        lines.append(
            f"| {item.name} | {item.category} | {baseline_val:.3f} {unit} | {current_val:.3f} {unit} | {delta} | {budget_display} | {status_icon} |"  # noqa: E501
        )
    lines.append("")
    return "\n".join(lines)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.20, help="Allowed slowdown ratio (e.g. 0.20 for 20%)."
    )
...
def main() -> None:
    ...
    if failures:
        raise SystemExit("Performance regression detected beyond allowed threshold")
