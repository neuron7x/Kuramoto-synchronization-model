...
def main():
    ...
    parser.add_argument(
        "metrics_file", type=Path, nargs="?", help="JSON file containing metrics to validate"
    )
    ...
    parser.add_argument(
        "--fail-fast", action="store_true", help="Exit immediately on first validation failure"
    )

    parser.add_argument(
        "--show-config", action="store_true", help="Display current configuration and exit"
    )
    ...
    if args.show_config:
        ...
        for metric in config.metrics:
            print(
                f"  {metric.name:20s}: {metric.threshold:8.3f} {metric.unit:5s} (weight: {metric.weight:.1f})"  # noqa: E501
            )
        return 0
