...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--locales-config", type=Path, default=DEFAULT_LOCALES_YAML, help="Path to locales.yaml"
    )
    parser.add_argument(
        "--translations-dir",
        type=Path,
    ...
    parser.add_argument(
        "--metadata-output", type=Path, default=METADATA_JSON, help="Generated metadata JSON path"
    )
    parser.add_argument(
        "--schema", type=Path, default=SCHEMA_PATH, help="JSON schema for translation bundles"
    )
    parser.add_argument(
        "--coverage-report", type=Path, default=COVERAGE_REPORT, help="Output coverage report path"
    )
    parser.add_argument(
        "--vendor-dir",
        type=Path,
    ...
    parser.add_argument(
        "--check", action="store_true", help="Validate only; fail if changes would be produced"
    )
