...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
  ...
  parser.add_argument('--locales-config', type=Path, default=DEFAULT_LOCALES_YAML, help='Path to locales.yaml')
  parser.add_argument('--translations-dir', type=Path, default=TRANSLATIONS_DIR, help='Path to locale JSON bundles')
  parser.add_argument('--metadata-output', type=Path, default=METADATA_JSON, help='Generated metadata JSON path')
  parser.add_argument('--schema', type=Path, default=SCHEMA_PATH, help='JSON schema for translation bundles')
  parser.add_argument('--coverage-report', type=Path, default=COVERAGE_REPORT, help='Output coverage report path')
  parser.add_argument('--vendor-dir', type=Path, default=DEFAULT_VENDOR_DIR, help='Directory with vendor-provided locale JSON overrides')
  parser.add_argument('--check', action='store_true', help='Validate only; fail if changes would be produced')
...
def validate_translation(payload: Dict[str, Any], validator: Draft202012Validator, path: Path) -> None:
...
def write_translations(translations: Dict[str, Dict[str, Any]], translations_dir: Path, check: bool) -> bool:
  wrote = False
...
def main(argv: Iterable[str]) -> int:
  ...
  if locales_config.get('default_locale') not in translations:
    raise RuntimeError(f"Default locale '{locales_config.get('default_locale')}' is missing from translations")
  ...
  for locale, payload in translations.items():
    ...
    coverage['locales'][locale] = {
      ...
      'coverageRatio': round((len(base_keys) - len(stats.missing)) / len(base_keys), 4) if base_keys else 1.0,
    }
  ...
  if args.check:
    pending_changes = vendor_changed or write_translations(translations, args.translations_dir, check=True)
    metadata_changed = not args.metadata_output.exists() or json.loads(args.metadata_output.read_text(encoding='utf-8')) != metadata_payload
    ...
    if issues:
      for locale, stats in issues:
        if stats.missing:
          print(f"Locale {locale} missing keys: {', '.join(sorted(stats.missing))}", file=sys.stderr)
        if stats.extra:
          print(f"Locale {locale} has unexpected keys: {', '.join(sorted(stats.extra))}", file=sys.stderr)
  ...
  if issues:
    for locale, stats in issues:
      ...
      if stats.extra:
        print(f"[warn] Locale {locale} has unexpected keys: {', '.join(sorted(stats.extra))}")
    raise SystemExit(1)
