...
def compare_circuit_breaker(
    ...
    return {
        ...
        "severity": (
            "critical" if "CRITICAL" in cb2 else "warning" if cb2 != "normal" else "ok"
        ),
    }


def generate_comparison_report(
    comparison: Dict[str, Any], state1_path: Path, state2_path: Path
) -> None:
    ...
    print(
        f"  Change:        {fe_comp['delta_F']:+.6f} ({fe_comp['percent_change']:+.2f}%)"
    )
    ...
    if fe_comp["F_after"] > 1.35:
        issues.append(
            f"❌ Free energy ({fe_comp['F_after']:.4f}) exceeds threshold (1.35)"
        )
    ...
    if issues:
        ...
        if fe_comp["status"] == "improved":
            print(
                f"    ✓ Free energy improved by {abs(fe_comp['percent_change']):.1f}%"
            )
...
def main():
    ...
    parser.add_argument("state1", type=Path, help="First state file (baseline/before)")

    parser.add_argument("state2", type=Path, help="Second state file (current/after)")

    parser.add_argument(
        "--output", "-o", type=Path, help="Output path for JSON comparison report"
    )
