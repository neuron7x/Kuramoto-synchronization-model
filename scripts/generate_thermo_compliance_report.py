...
def generate_text_report(
    analysis: Dict[str, Any], start_date: datetime, end_date: datetime, output_path: Path
) -> None:
    ...
    report_lines = [
        ...
        f"Energy Threshold (1.35):        {'COMPLIANT' if analysis['max_free_energy'] <= 1.35 else 'NON-COMPLIANT'}",  # noqa: E501
        "",
        "=" * 70,
        "SAFETY EVENTS",
        "=" * 70,
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", type=Path, required=True, help="Output file path (.txt or .pdf)"
    )
