...
try:  # NumPy <2.0 exposed RankWarning at the top level
    from numpy import RankWarning as _RankWarning
except (ImportError, AttributeError):  # pragma: no cover - depends on NumPy minor version
    from numpy.polynomial.polyutils import RankWarning as _RankWarning  # type: ignore

EPS: float = 1e-10
EPS_PROB_CLIP: float = 1e-5
TAU_MIN: float = 0.5
TAU_MAX: float = 2.0
RISK_FREE_RATE: float = 0.02  # 2% annualised risk-free rate used for Sharpe
...
class MultiScaleRiskMonitor:
    ...
    def update(self, equity: float) -> Dict[str, float]:
        ...
        result: Dict[str, float] = {"current_dd": float(current_dd), "peak": float(self.peak)}
...
class FETE:
    ...
    def reset_state(self) -> None:
        """Reset internal analytics and history for a fresh evaluation run."""
    ...
    def backtest(
        ...
        *,
        ...
        prob_array = np.clip(prob_array, EPS_PROB_CLIP, 1.0 - EPS_PROB_CLIP)
        equity = 1.0
        equity_curve = [equity]
        ...
        if eq_returns.size > 2 and np.std(eq_returns) > 0:
            sharpe = float(
                np.sqrt(252.0)
                * (np.mean(eq_returns) - (RISK_FREE_RATE / 252.0))
