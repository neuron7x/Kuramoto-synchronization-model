...
from core.data.market_feed import MarketFeedRecord, MarketFeedMetadata, MarketFeedRecording


MarketRegime = Literal["trending_up", "trending_down", "mean_reverting", "volatile", "stable"]
...
class SyntheticMarketFeedGenerator:
    ...
    def generate(
        ...
        for i in range(num_records):
            ...
            price_change = drift * current_price + self.rng.normal(0, volatility * current_price)
    ...
    def generate_flash_crash(
        ...
        for i in range(num_records):
            ...
            latency = max(
                0.001, self.rng.normal(self.latency_mean_ms, self.latency_std_ms) / 1000.0
            )
            ingest_time = current_time + timedelta(seconds=latency)
    ...
    def generate_regime_transition(
        ...
        transition_indices = [int(p * num_records) for p in transition_points] + [num_records]
        start_idx = 0
        ...
        for regime, end_idx in zip(regimes, transition_indices):
            ...
            if segment_size > 0:
                ...
                generator = SyntheticMarketFeedGenerator(
                    ...
                    base_price=self.base_price if not all_records else float(all_records[-1].last),
