...
class MarketFeedRecord(BaseModel):
    ...
    def _validate_prices(self) -> "MarketFeedRecord":
        ...
        if latency_ms > 10_000:
            raise ValueError(f"Latency {latency_ms:.1f}ms exceeds maximum threshold of 10s")
...
def validate_recording(recording: MarketFeedRecording) -> Dict[str, Any]:
    ...
    for i in range(1, len(recording)):
        gap_ms = (recording[i].exchange_ts - recording[i - 1].exchange_ts).total_seconds() * 1000
    ...
    if volume_zero_count > len(recording) * 0.5:
        warnings.append(f"High proportion of zero volume: {volume_zero_count}/{len(recording)}")
    ...
    return {
        ...
        "duration_seconds": (recording[-1].exchange_ts - recording[0].exchange_ts).total_seconds(),
        "latency_ms": {
