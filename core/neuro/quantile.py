...
class ExactQuantile:
    ...
    def __init__(self, q: float) -> None:
        ...
        if not (0.0 < q < 1.0):
            raise ValueError(
                f"Quantile must be in open interval (0, 1), got {q}"
            )
        self.p = float(q)
    ...
    def _linear(self, i: int, d: float) -> float:
        ...
        if d > 0:
            return self._markers[i] + \
                   (self._markers[i + 1] - self._markers[i]) / (self._n[i + 1] - self._n[i])
        else:
            return self._markers[i] - \
                   (self._markers[i] - self._markers[i - 1]) / (self._n[i] - self._n[i - 1])
