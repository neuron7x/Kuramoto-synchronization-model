...
if authentication_status is False:
        ...
        if uploaded:
            try:
                ...
                if "price" in df.columns:
                    ...
                    if missing_prices > 0:
                        st.warning(
                            f"⚠️ Found {missing_prices} missing price values ({missing_prices/len(df)*100:.1f}%)"  # noqa: E501
                        )
        ...
        if uploaded and "price" in df.columns:
            try:
                ...
                if len(prices) < window_size:
                    st.warning(
                        f"Data has {len(prices)} rows but window size is {window_size}. Using all available data."  # noqa: E501
                    )
                ...
                with col1:
                    st.metric(
                        "Kuramoto Order (R)",
                        f"{R:.4f}",
                        help="Measures phase synchronization. Higher values indicate stronger coherence.",  # noqa: E501
                    )
                with col2:
                    st.metric(
                        f"Entropy H({analysis_window})",
                        f"{H:.4f}",
                        help="Shannon entropy of price distribution. Higher values indicate more uncertainty.",  # noqa: E501
                    )
                with col3:
                    st.metric(
                        f"Delta Entropy ΔH({analysis_window})",
                        f"{dH:.4f}",
                        help="Change in entropy over the window. Indicates shifting market dynamics.",  # noqa: E501
                    )
                with col4:
                    st.metric(
                        "Hurst Exponent",
                        f"{Hs:.4f}",
                        help="Measures long-term memory. H>0.5: trending, H<0.5: mean-reverting, H≈0.5: random walk",  # noqa: E501
                    )
                ...
                with col1:
                    st.metric(
                        ...
                        f"{kappa:.6f}",
                        help="Geometric curvature of price manifold. Negative values indicate expanding regimes.",  # noqa: E501
                    )
                with col2:
                    ...
                    st.metric(
                        "Regime Type", regime_type, help="Based on Hurst exponent classification"
                    )
                ...
                with st.expander("📊 Detailed Analysis Summary"):
                    ...
                    if abs(Hs - 0.5) > 0.15:
                        predictability = "High"
                    elif abs(Hs - 0.5) > 0.05:
                        predictability = "Moderate"
                    else:
                        predictability = "Low"

                    if kappa > 0:
                        manifold = "Contracting"
                    elif kappa < -0.001:
                        manifold = "Expanding"
                    else:
                        manifold = "Stable"
                ...
                st.info(
                    "This may be due to insufficient data or invalid values. Please check your dataset."  # noqa: E501
                )
    ...
    with tab3:
        ...
        if len(st.session_state.analysis_history) > 0:
            ...
            st.info(
                "No analysis history yet. Analyze some data in the 'Indicators' tab to build history."  # noqa: E501
            )
