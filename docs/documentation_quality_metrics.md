...
1. **CI Workflows** – `docs-metrics-nightly.yml` (planned) orchestrates nightly
   runs of the metadata, freshness, link health, and snippet replay checks.
   Quarterly freshness snapshots are generated by `docs-freshness.yml` using
   `scripts/docs/check_freshness.py`.
