...
TradePulse runs a quarterly documentation review ritual to keep content current and to provide a documented audit trail of
freshness status.

**Automated Tracking**
- GitHub Actions workflow `docs-freshness.yml` runs quarterly and produces a freshness report.
- The report is generated by `scripts/docs/check_freshness.py` and uploaded as a workflow artifact.
- The Documentation Steward files a follow-up issue for any stale items, linking to the artifact and tagging domain owners.

**Ritual Steps**
1. **Run the freshness snapshot** – Execute `python scripts/docs/check_freshness.py --format markdown --output reports/docs/quarterly/<YYYY-Qn>.md`.
2. **Triage stale documents** – Tag owners for items overdue beyond their `review_cadence` and update `DOCUMENTATION_SUMMARY.md`.
3. **Schedule reviews** – Add calendar invites for owners with >2 overdue documents.
4. **Close the loop** – Record outcomes in the quarterly report and update stale documents' metadata after review.
