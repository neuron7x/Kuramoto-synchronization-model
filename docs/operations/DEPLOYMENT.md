...
This guide outlines how to run GeoSync with Docker Compose for workstation and staging setups and how to prepare a Helm-based deployment for Kubernetes clusters. It also explains how to manage sensitive configuration and wire health checks into your automation pipelines. For a visual overview of the services involved in each environment, refer to the [architecture diagrams](../architecture/system_overview.md) and [feature-store topology](../architecture/feature_store.md) before proceeding with environment-specific steps.
...
- Access to a container registry where you can push the GeoSync image, if you plan to deploy to Kubernetes.
...
⚠️ **CRITICAL**: As of 2025-11-17, the security constraint policy has been significantly enhanced to address a critical supply chain vulnerability. See [SECURITY_CONSTRAINT_POLICY.md](../security/dependency_policy.md) for full details.
...
- Commit the updated constraint and regenerated lock files. Dependabot is configured to watch `/constraints/security.txt`, so
  ...
  See [docs/SECURITY_DEPENDENCIES.md](../SECURITY_DEPENDENCIES.md) for the complete dependency hardening playbook.
...
- Export the corresponding environment variables that the Hydra experiments expect (`PROD_DB_CA_FILE`, `PROD_DB_CERT_FILE`,
  `PROD_DB_KEY_FILE`, and their staging equivalents). These defaults map to `/etc/geosync/db/*.pem` paths in the sample
...
3. **Rollback**: Use `kubectl rollout undo deployment/geosync-api -n <env>` (or `helm rollback <release> <rev>`) after capturing Prometheus snapshots; rerun `pytest tests/smoke -m smoke` as a post-rollback verification.  
...
The repository ships with a lightweight Compose stack that builds the GeoSync container and runs Prometheus for metrics scraping.【F:docker-compose.yml†L1-L12】

1. **Build images** (only required when you change the application code):
   ```bash
   docker compose build geosync
   ```
...
The GeoSync service exposes an HTTP health endpoint at `/health` on port 8000 by default. The docker-compose.yml includes a healthcheck that integrates with Docker's health status reporting:
...
The default port can be customized using the `GEOSYNC_HTTP_PORT` environment variable. For example, to use port 8001:

```bash
export GEOSYNC_HTTP_PORT=8001
docker compose up -d
```
...
Prometheus runs on port 9090 by default. Set `GEOSYNC_PROMETHEUS_PORT` before invoking `docker compose` to bind it to an alternate host port, or let the smoke test script choose a free port automatically when 9090 is already in use. The script propagates the selected port to its Prometheus probes so that runtime diagnostics keep working even on shared CI runners.

## Kubernetes Infrastructure with Terraform and Kustomize

GeoSync now ships with first-class infrastructure code for Amazon EKS alongside Kustomize overlays for staging and production workloads. Use Terraform to provision the cluster(s) and managed node groups, then deploy the application manifests with the provided overlays.
...
kubectl rollout status deployment/geosync-api -n geosync-staging

kubectl apply -k deploy/kustomize/overlays/production
kubectl rollout status deployment/geosync-api -n geosync-production
```

Secrets referenced by the deployments (`geosync-secrets`, `geosync-mtls-client`) must be provisioned outside of source control via your secret management workflow.

### Continuous Delivery Pipeline

The `Deploy GeoSync Environments` GitHub Actions workflow automates validation and rollouts for both environments.【F:.github/workflows/deploy-environments.yml†L1-L139】 Key characteristics:
...
- **Kubernetes** – create Secrets straight from the same environment file:
  ```bash
  kubectl create secret generic geosync-secrets \
    --from-env-file=.env \
    --namespace geosync
  ```
...
- Rotate API keys and credentials regularly. Update the Secret object and restart the workloads (`kubectl rollout restart deployment geosync`).
...
- **HashiCorp Vault** – set `GEOSYNC_VAULT_ADDR` with either
  `GEOSYNC_VAULT_TOKEN` or `GEOSYNC_VAULT_TOKEN_FILE`. Optional knobs like
  `GEOSYNC_VAULT_NAMESPACE`, `GEOSYNC_VAULT_MOUNT`, and
  `GEOSYNC_VAULT_KV_VERSION` control namespaces and KV engine behaviour.
  Audit metadata can be overridden via
  `GEOSYNC_VAULT_AUDIT_ACTOR` / `GEOSYNC_VAULT_AUDIT_IP`.
- **AWS Secrets Manager** – enable the integration with
  `GEOSYNC_AWS_SECRETS_MANAGER_ENABLED=true` and provide a region through
  `GEOSYNC_AWS_SECRETS_REGION`. TLS and endpoint overrides are respected via
  `GEOSYNC_AWS_SECRETS_VERIFY` and `GEOSYNC_AWS_SECRETS_ENDPOINT`.
...
- **Prometheus** – keep the scrape configuration aligned with your target service names (`geosync:8001` in Docker Compose, `<service-name>:8001` in Kubernetes).【F:deploy/prometheus.yml†L2-L7】
...
GeoSync releases are automated through GitHub Actions:
...
# create a signed SemVer tag and push it
 git tag -s vX.Y.Z -m "GeoSync vX.Y.Z"
