...
This guide provides step-by-step instructions for setting up the GeoSync development environment locally or in a container.
...
GeoSync uses a Python virtual environment to isolate dependencies.
...
GeoSync uses environment variables for configuration. A template is provided in `.env.example`.
...
GEOSYNC_AUDIT_SECRET=<generate_secure_secret_minimum_16_chars>
GEOSYNC_RBAC_AUDIT_SECRET=<generate_secure_secret_minimum_16_chars>
...
python -c "import secrets; print('GEOSYNC_AUDIT_SECRET=' + secrets.token_hex(32))"
python -c "import secrets; print('GEOSYNC_RBAC_AUDIT_SECRET=' + secrets.token_hex(32))"
```
...
docker compose up -d geosync prometheus
```
...
# Test GeoSync service
curl -k https://localhost:8000/health

# Access services:
# - GeoSync API: https://localhost:8000
...
docker compose exec geosync python -m interfaces.cli --help
```
...
|----------|----------|---------|-------------|
| `GEOSYNC_ENV` | Yes | `development` | Environment: `development`, `staging`, `production` |
| `LOG_LEVEL` | No | `INFO` | Logging level: `DEBUG`, `INFO`, `WARNING`, `ERROR` |
| `GEOSYNC_AUDIT_SECRET` | Yes | — | Secret for signing audit records (min 16 chars) |
| `GEOSYNC_RBAC_AUDIT_SECRET` | Yes | — | Secret for RBAC audit (min 16 chars) |
| `SECRET_KEY` | Yes | — | Application secret key |
| `JWT_SECRET` | Yes | — | JWT token signing secret |
| `POSTGRES_HOST` | No | `localhost` | PostgreSQL host |
| `POSTGRES_PORT` | No | `5432` | PostgreSQL port |
| `POSTGRES_USER` | No | `geosync` | PostgreSQL username |
| `POSTGRES_PASSWORD` | Yes | — | PostgreSQL password |
| `POSTGRES_DB` | No | `geosync` | PostgreSQL database name |
...
grep -E "(GEOSYNC_AUDIT_SECRET|GEOSYNC_RBAC_AUDIT_SECRET)" .env
...
- 📖 Check [docs/troubleshooting.md](../troubleshooting.md) for more solutions
- 💬 Ask in [GitHub Discussions](https://github.com/neuron7xLab/GeoSync/discussions)
- 🐛 Report issues on [GitHub Issues](https://github.com/neuron7xLab/GeoSync/issues)
