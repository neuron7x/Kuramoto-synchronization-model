...
class TestNaKController(unittest.TestCase):
    def setUp(self) -> None:
        self._seed_env_previous = os.environ.get("NAK_SEED")
        os.environ["NAK_SEED"] = "1337"
        self.hook = NaKHook(CONFIG_PATH, seed=1337)

    def tearDown(self) -> None:
        if self._seed_env_previous is None:
            os.environ.pop("NAK_SEED", None)
        else:
            os.environ["NAK_SEED"] = self._seed_env_previous
    ...
    def test_modes_and_hysteresis(self) -> None:
        ...
        out_rec: Dict[str, object] | None = None
        for _ in range(6):
            out_rec = self._step(gv=0.2, pd=0.02, pnl=0.003, trades=0.2, lvol=0.1)
        if out_rec is None:
            self.fail("expected recovery observation, got None")
