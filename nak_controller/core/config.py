...
class NakConfig(BaseModel):
    ...
    def _validate_hysteresis(cls, value: float) -> float:
        if value <= 0.0:
            raise ValueError("EI_hysteresis must be strictly positive")
        return value
    ...
    @classmethod
    def _risk_band(cls, value: float, info: ValidationInfo) -> float:
    ...
    def _cross_field_checks(self) -> "NakConfig":
        weights_total = self.w_n + self.w_v + self.w_d + self.w_e + self.w_l + self.w_s
        if weights_total > 1.0 + 1e-9:
            raise ValueError("sum of load weights must not exceed 1.0")
        if not (self.r_min < self.r_max):
            raise ValueError("risk band must have increasing bounds")
        if not (self.r_min <= 1.0 <= self.r_max):
            raise ValueError("risk band must include the neutral factor 1.0")
        return self
