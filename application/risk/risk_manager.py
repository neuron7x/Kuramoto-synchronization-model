...
class RiskManagerFacade:
    ...
    def engage_kill_switch(
        ...
        *,
        ...
        # Read the (triggered, reason) pair atomically so we never act on a torn
        # snapshot (engaged flag from one state change, reason from another).
        already_engaged, previous_reason = kill_switch.snapshot()
        ...
        engaged, current_reason_raw = kill_switch.snapshot()
        current_reason = current_reason_raw or previous_reason or normalised_reason
    ...
    def reset_kill_switch(
        ...
        *,
        ...
        kill_switch = self._risk_manager.kill_switch
        was_engaged, previous_reason = kill_switch.snapshot()
        if was_engaged or bool(previous_reason):
            kill_switch.reset()
        engaged, _ = kill_switch.snapshot()
        return KillSwitchState(
            engaged=engaged,
    ...
    def kill_switch_state(
        ...
        *,
        ...
        kill_switch = self._risk_manager.kill_switch
        engaged, reason = kill_switch.snapshot()
        return KillSwitchState(
            engaged=engaged,
            reason=reason,
