...
"""Access control primitives for sensitive GeoSync operations."""
...
class Permission:
    ...
    def allows(self, action: str, resource: str | None) -> bool:
        """True iff this permission grants ``action`` on ``resource`` (``*`` matches any)."""
...
class AccessController:
    ...
    def is_allowed(
        ...
        *,
        ...
        """Return whether ``actor`` (or a matching role, else the fallback subject) may perform
        ``action`` on ``resource``. Blank actions are rejected; unknown identifiers deny."""
    ...
    def require(
        ...
        *,
        ...
        """Raise :class:`AccessDeniedError` unless :meth:`is_allowed` grants the action."""
