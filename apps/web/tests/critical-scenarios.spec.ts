...
async function gotoScenarioStudio(page: Page, { clipboardMode }: { clipboardMode?: ClipboardMode } = {}) {
...
async function expectClipboardValue(page: Page, expected: string) {
  const actual = await page.evaluate(() => (window as typeof window & { __lastCopied?: string }).__lastCopied ?? '');
...
test.describe('Critical customer journeys', () => {
  ...
  test('processes purchases via JSON downloads', async ({ page }) => {
    ...
    await expect(page.getByTestId('action-feedback')).toContainText('Scenario JSON download started.');
  });
  ...
  test('surfaces refunds-style messaging when exports fail and recovers on retry', async ({ page }) => {
  ...
  test('captures a visual regression baseline for the production-ready view', async ({ page }) => {
    ...
    test.skip(!projectName.includes('Desktop Chrome'), 'Visual baseline captured on desktop Chromium only');
