...
COMPONENTS: tuple[Component, ...] = (
    ...
    Component(
        ...
        test_body=dedent(
            ...
            expected_keys = {
                "corr", "mean_precision", "precision_std", "pulse_std", "pe_std", "score"
            }
    ...
    Component(
        ...
        test_body=dedent(
            """
            cfg = CalibConfig(
                iters=5, seed=2, ema_span=(4, 5), vol_lambda=(0.8, 0.81),
                alpha=(0.3, 0.31), beta=(0.2, 0.21), lambda_sync=(0.5, 0.51),
                eta_ricci=(0.1, 0.11), rho=(0.02, 0.021)
            )
            ...
            if np.isfinite(result.score):
                expected_score = result.metrics["corr"] * result.metrics["mean_precision"]
                assert np.isclose(result.score, expected_score)

            empty_cfg = CalibConfig(iters=0, seed=cfg.seed)
            empty_result = calibrate_random(x, R, kappa, empty_cfg, return_details=True)
