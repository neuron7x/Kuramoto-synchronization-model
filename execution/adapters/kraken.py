...
class KrakenRESTConnector(RESTWebSocketConnector):
    ...
    def healthcheck(self, credentials: Mapping[str, str] | None = None) -> ConnectorHealth:
        ...
        try:
            ...
            try:
                ...
                if response.status_code != 200:
                    ...
                    rest_detail = "REST time probe succeeded"
            ...
            try:
                ...
                try:
                    ...
                    start = time.perf_counter()
                    params, _, headers, data = self._sign_request(
                        "POST",
                        "/0/private/Balance",
                        params={},
                        json_payload=None,
                    ...
                    if response.status_code == 200 and not payload.get("error"):
                        ...
                        auth_detail = "Authenticated balance request succeeded"
                    ...
                    if auth_status is HealthStatus.OK:
                        token_params, _, token_headers, token_data = self._sign_request(
                            "POST",
                        ...
                        token_payload = token_response.json() if token_response.text else {}
                        token_result = token_payload.get("result") or {}
                        candidate = token_result.get("token") if isinstance(token_result, Mapping) else None
                        if token_response.status_code == 200 and candidate:
                            token = str(candidate)
                        else:
                            auth_status = HealthStatus.WARN
                            auth_detail = "Failed to acquire WebSocket token"
                except Exception as exc:  # pragma: no cover - defensive guard
                    auth_status = HealthStatus.FAIL
        ...
        probe = ws_probe
        public_stream = self._stream_base.replace("ws-auth", "ws")
        if probe is None:
        ...
        try:
            start = time.perf_counter()
            messages, intervals = asyncio.run(probe(public_stream, 3, 5.0))
        ...
        checks.append(
            HealthCheck(
                ...
                data={"intervals_ms": ws_intervals, "token_acquired": bool(token)},
            )
        )
