...
spec_ecs = importlib.util.spec_from_file_location(
    "core.neuro.ecs_regulator", Path(__file__).parent.parent / "core" / "neuro" / "ecs_regulator.py"
)
...
def simulate_integrated_trading(n_steps: int = 100, seed: int = 42):
    ...
    ecs_reg = ECSInspiredRegulator(
        initial_risk_threshold=0.05, stress_threshold=0.1, chronic_threshold=5, seed=seed
    )
