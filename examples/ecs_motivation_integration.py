...
def simulate_integrated_trading(n_steps: int = 100, seed: int = 42):
    ...
    print("  ECS Only:")
    print(
        f"    Sells: {ecs_counts['sell']:3d} ({ecs_counts['sell'] / n_steps * 100:.1f}%)"
    )
    print(
        f"    Holds: {ecs_counts['hold']:3d} ({ecs_counts['hold'] / n_steps * 100:.1f}%)"
    )
    print(
        f"    Buys:  {ecs_counts['buy']:3d} ({ecs_counts['buy'] / n_steps * 100:.1f}%)"
    )
    print()
