...
class TestInputValidation:
    ...
    def test_string_input_sanitization(self):
        ...
        for malicious in malicious_inputs:
            ...
            assert (
                "DROP TABLE" not in sanitized.upper()
            ), f"DROP TABLE found in: {sanitized}"
            assert (
                "UNION SELECT" not in sanitized.upper()
            ), f"UNION SELECT found in: {sanitized}"
            assert (
                "<script>" not in sanitized.lower()
            ), f"<script> found in: {sanitized}"
