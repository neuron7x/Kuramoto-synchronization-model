...
def test_CalibConfig_autogenerated() -> None:
    """Validates that the calibration configuration advertises the expected default search space for AdaptiveMarketMind parameters."""  # noqa: E501
    cfg = CalibConfig()
...
def test_rand_helper_autogenerated() -> None:
    """Ensures the internal ``_rand`` helper respects the provided boundaries for both integer and floating-point draws."""  # noqa: E501
...
def test_evaluate_trace_autogenerated() -> None:
    """Covers ``_evaluate_trace`` behaviour for degenerate inputs and verifies that a well-formed trace yields the documented metrics."""  # noqa: E501
    short = _evaluate_trace(
        np.array([0.1], dtype=Float), np.array([1.0], dtype=Float), np.array([1.0], dtype=Float)
    )
...
def test_calibrate_random_autogenerated() -> None:
    """Exercises ``calibrate_random`` end-to-end to assert bounds, default fallbacks, and the integrity of the returned diagnostics."""  # noqa: E501
    cfg = CalibConfig(
        iters=5,
        seed=2,
        ema_span=(4, 5),
