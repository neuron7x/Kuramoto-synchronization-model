...
def test_publish_job_runs_only_on_pushes_to_main() -> None:
    workflow = _load_ci_workflow()
    job = _get_publish_job(workflow)
    assert job["if"].strip() == "github.event_name == 'push' && github.ref == 'refs/heads/main'"
...
def test_publish_job_defines_expected_environment_variables() -> None:
    ...
    assert env["GHCR_IMAGE"].startswith("ghcr.io/")
