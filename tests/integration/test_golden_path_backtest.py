...
def mean_reverting_price_data() -> np.ndarray:
    ...
    for i in range(1, n_bars):
        ...
        prices[i] = (
            prices[i - 1] + reversion_speed * (mean_price - prices[i - 1]) + innovation
        )
...
class TestGoldenPathBasic:
    ...
    def test_backtest_produces_valid_result(
        self, synthetic_price_data: np.ndarray
    ) -> None:
    ...
    def test_equity_curve_invariants(self, synthetic_price_data: np.ndarray) -> None:
        ...
        ), "Equity curve contains non-finite values"
...
class TestGoldenPathStrategies:
    ...
    def test_buy_and_hold_on_trending_data(
        self, trending_price_data: np.ndarray
    ) -> None:
...
class TestGoldenPathDataValidation:
    ...
    def test_validation_config_respected(
        self, synthetic_price_data: np.ndarray
    ) -> None:
...
class TestGoldenPathAntiLeakage:
    ...
    def test_anti_leakage_config_applies(
        self, synthetic_price_data: np.ndarray
    ) -> None:
...
class TestGoldenPathDeterminism:
    ...
    def test_repeated_runs_produce_same_result(
        self, synthetic_price_data: np.ndarray
    ) -> None:
