...
pytestmark = pytest.mark.skip(
    reason="Tests need refactoring to match async adapter interface"
)
...
def polygon_adapter(mock_polygon_client):
    ...
    with patch(
        "core.data.adapters.polygon.RESTClient", return_value=mock_polygon_client
    ):
...
class TestDataFetching:
    ...
    def test_fetch_bars_returns_dataframe(self, polygon_adapter):
        ...
        result = polygon_adapter.fetch_bars(
            symbol="AAPL", start="2024-01-01", end="2024-01-02", timeframe="1h"
        )
    ...
    def test_fetch_bars_validates_symbol(self, polygon_adapter):
        ...
        with pytest.raises(ValueError, match="Symbol .* required"):
            polygon_adapter.fetch_bars(symbol="", start="2024-01-01", end="2024-01-02")
    ...
    def test_fetch_bars_validates_date_range(self, polygon_adapter):
        ...
        with pytest.raises(ValueError, match="Start date .* after end date"):
            polygon_adapter.fetch_bars(
                symbol="AAPL", start="2024-01-10", end="2024-01-01"
            )

    def test_fetch_bars_handles_empty_response(
        self, polygon_adapter, mock_polygon_client
    ):
        ...
        result = polygon_adapter.fetch_bars(
            symbol="INVALID", start="2024-01-01", end="2024-01-02"
        )
...
class TestErrorHandling:
    ...
    def test_fetch_bars_retries_on_rate_limit(
        self, polygon_adapter, mock_polygon_client
    ):
    ...
    def test_fetch_bars_handles_malformed_response(
        self, polygon_adapter, mock_polygon_client
    ):
        """Test fetch_bars handles malformed API response."""
        mock_polygon_client.get_aggs.return_value = {
            "results": [{"o": 100.0, "h": 110.0}],  # Missing required fields
            "status": "OK",
        }
...
class TestRateLimiting:
    ...
    def test_backoff_strategy_on_rate_limit(self, polygon_adapter, mock_polygon_client):
        ...
        with patch("time.sleep") as mock_sleep:
            result = polygon_adapter.fetch_bars(
                symbol="AAPL", start="2024-01-01", end="2024-01-02", max_retries=3
            )
...
class TestCaching:
    ...
    def test_cache_stores_fetched_data(self, polygon_adapter, mock_polygon_client):
        ...
        polygon_adapter.fetch_bars(
            symbol="AAPL", start="2024-01-01", end="2024-01-02", use_cache=True
        )
    ...
    def test_cache_can_be_bypassed(self, polygon_adapter, mock_polygon_client):
        ...
        # Fetch without cache
        polygon_adapter.fetch_bars(
            symbol="AAPL", start="2024-01-01", end="2024-01-02", use_cache=False
        )
