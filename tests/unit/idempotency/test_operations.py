...
def test_key_factory_attempt_changes_operation_id(key_factory: IdempotencyKeyFactory) -> None:
...
def test_coordinator_records_successful_replay(key_factory: IdempotencyKeyFactory) -> None:
    clock = ManualClock()
...
def test_coordinator_detects_conflicting_payloads(key_factory: IdempotencyKeyFactory) -> None:
...
def test_coordinator_rejects_retry_after_failure(key_factory: IdempotencyKeyFactory) -> None:
    coordinator = IdempotencyCoordinator()
    key = key_factory.build(service="orders", operation="submit", dedupe_fields={"id": 1})
    coordinator.register_attempt(key)
...
def test_metrics_track_duplicates_and_collisions(key_factory: IdempotencyKeyFactory) -> None:
    coordinator = IdempotencyCoordinator()
    key = key_factory.build(service="orders", operation="submit", dedupe_fields={"id": 1})
    coordinator.register_attempt(key)
    coordinator.complete_success(key, {"status": "ok"})
