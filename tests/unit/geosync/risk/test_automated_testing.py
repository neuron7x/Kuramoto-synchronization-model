...
class TestAutomatedRiskTester:
    ...
    def test_run_stress_test_passed_flag_tracks_zero_errors(self):
        """`passed = len(validation_errors) == 0` -- a clean run passes.

        Under Eq->NotEq the flag inverts: a scenario with no validation errors
        would be reported as FAILED. A clean normal scenario pins it.
        """
        tester = AutomatedRiskTester(seed=42)
        np.random.seed(42)
        scenario = RiskScenario(
            ...
            description="clean normal market",
        )
        result = tester.run_stress_test(scenario)
    ...
    def test_calculate_sharpe_ratio_empty_returns(self):
        """`if len(returns) == 0: return 0.0` -- an empty series has no Sharpe.

        Under Eq->NotEq the guard skips the empty case and computes on an empty
        array, yielding NaN. Pins the exact empty-length branch (distinct from
        the zero-volatility test, which passes a non-empty constant series).
        """
        sharpe = AutomatedRiskTester._calculate_sharpe_ratio(np.array([], dtype=float))
        assert sharpe == 0.0
...
class TestValidateRiskMetrics:
    ...
    def test_validate_risk_metrics_reports_nonzero_sharpe_for_variance(self):
        """`sharpe = ... if sigma2 > 0 else 0.0` -- positive variance yields a
        real Sharpe.

        Under Gt->LtE the guard inverts: a return series WITH variance would
        report a zeroed Sharpe. Normal returns (non-zero variance) pin it.
        """
        np.random.seed(7)
        returns = np.random.normal(0.001, 0.01, 252)
        result = validate_risk_metrics(returns, alpha=0.975, es_limit=0.03)
