...
"""The event-bus lifecycle matrix must bind every invariant to a real test.

The event bus is a runtime inference substrate: an untracked async task, a
swallowed handler exception, or a publish after stop is a silent lie. This gate
keeps the lifecycle matrix honest — schema-valid, >= 8 invariants, each bound to
an existing PASS test node.
"""
...
MATRIX = ROOT / "artifacts" / "messaging" / "event_bus_lifecycle_matrix.json"
SCHEMA = ROOT / "audit" / "schema" / "event_bus_lifecycle_matrix.schema.json"
