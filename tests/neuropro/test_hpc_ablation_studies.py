...
class TestComponentContributions:
    ...
    def test_blending_contribution_to_stability(self):
        ...
        print(
            f"\nReward variance with blending: {np.mean(variances_blend):.6f} ± {np.std(variances_blend):.6f}"  # noqa: E501
        )
        print(
            f"Reward variance expert-only: {np.mean(variances_expert):.6f} ± {np.std(variances_expert):.6f}"  # noqa: E501
        )
