...
def test_custom_gate_params():
    ...
    assert hasattr(metrics, 'plasticity_delta')
...
def test_gate_metrics_type():
    ...
    assert isinstance(metrics.plasticity_delta, float)
...
def test_realized_volatility_contributes_to_drive():
    """When weighted toward realized vol, higher vol should raise inhibition."""

    params = GateParams(vix_drive_weight=0.0, vol_norm=0.1, vol_clip_max=5.0)
    gate = GABAInhibitionGate(params=params)
    action = torch.tensor([1.0])

    _, low_metrics = gate(base_state(vix=10.0, vol=0.05), action)
    gate.reset_state()
    _, high_metrics = gate(base_state(vix=10.0, vol=0.8), action)

    assert high_metrics.gaba_level > low_metrics.gaba_level
    assert high_metrics.inhibition > low_metrics.inhibition
...
def test_plasticity_delta_is_clamped():
    """Per-step plasticity contributions must respect the configured clamp."""

    params = GateParams(plasticity_max_delta=0.02, risk_max=5.0)
    gate = GABAInhibitionGate(params=params)
    action = torch.tensor([1.0])

    potentiating_state = base_state(
        vix=80.0, vol=1.5, ret=1.2, rpe=5.0, pos=0.0, dt_ms=5.0
    )
    _, potent_metrics = gate(potentiating_state, action)

    depressing_state = base_state(
        vix=80.0, vol=1.5, ret=-1.2, rpe=-5.0, pos=4.0, dt_ms=-5.0
    )
    _, depress_metrics = gate(depressing_state, action)

    assert abs(potent_metrics.plasticity_delta) <= params.plasticity_max_delta + 1e-6
    assert abs(depress_metrics.plasticity_delta) <= params.plasticity_max_delta + 1e-6
