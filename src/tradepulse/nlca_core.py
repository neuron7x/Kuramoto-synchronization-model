...
class NLCA:
    ...
    def sanitize_input(self, tick: dict):
        required = ["p_a1", "p_b1", "q_a", "q_b", "events", "messages", "trades", "delta_P", "Q"]
        for k in required:
    ...
    def step(self, tick: dict) -> dict:
        ...
        self.update_history(
            tick_sync["delta_P"], tick_sync["Q"], S_val, D_val, OFI_val, lam_val, OTR_val, BRHL_val
        )
        ...
        dummy_trades = [
            {"profit": np.random.normal(100, 10), "slippage": np.random.normal(0.01, 0.001)}
        ]
