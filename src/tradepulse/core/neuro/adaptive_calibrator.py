...
class AdaptiveCalibrator:
    ...
    def step(self, metrics: CalibrationMetrics) -> Dict[str, Any]:
        ...
        accept_prob = (
            np.exp(np.clip(delta / self.state.temperature, -20, 0))
            if delta < 0
            else 1.0
        )
    ...
    def get_calibration_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "iterations_since_improvement": self.state.iteration
            ...
            "recent_performance": {
                ...
                "dopamine_stability": np.mean(
                    [m.dopamine_stability for m in recent_metrics]
                ),
            },
            "exploration_state": (
                "exploring" if self.state.temperature > 0.5 else "exploiting"
            ),
