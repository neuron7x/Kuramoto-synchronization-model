...
class NeuroOrchestrator:
    ...
    def _build_learning_loop(
        ...
        return LearningLoop(
            ...
            update_rule="delta = reward + gamma * V_next - V_current; V_current += learning_rate * delta",  # noqa: E501
        )
    ...
    def _validate_free_energy_constraint(self, parameters: Dict[str, Any]) -> None:
        ...
        if not tacl_config.get("monotonic_descent", True):
            raise ValueError(
                "TACL monotonic_descent must be enabled to ensure free-energy constraint compliance."  # noqa: E501
            )
