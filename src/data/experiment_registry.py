...
class ExperimentRunRecord(BaseModel):
    ...
    experiment_name: str = Field(..., description="Name of the experiment the run belongs to.")
    ...
    metrics: dict[str, float] = Field(
        default_factory=dict, description="Evaluation metrics captured at the end of the run."
    )
    ...
    notes: str | None = Field(default=None, description="Free-form notes describing the run.")
    ...
    def audit_trail(self, experiment_name: str) -> list[HyperparameterAuditEntry]:
        ...
        for record in runs:
            entry = HyperparameterAuditEntry(
                ...
                changed_params=_diff_params(previous.params if previous else {}, record.params),
                metrics_delta=_metrics_delta(previous.metrics if previous else {}, record.metrics),
            )
