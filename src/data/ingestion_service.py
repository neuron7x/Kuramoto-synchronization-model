"""Data ingestion orchestration with dedicated read/write services."""
...
class _CacheState:
    """Container holding shared cache state for command/query services."""

    ingestor: DataIngestionService
    registry: CacheRegistry
    clock: Callable[[], datetime]
    metadata: dict[CacheKey, CacheEntrySnapshot]
    integrity_validator: TickFrameIntegrityValidator


class _DataIngestionBase:
    """Utility mixin shared between command and query services."""
...
class DataIngestionCommandService(_DataIngestionBase):
    """Transactional operations that mutate the ingestion cache."""
    ...
    def ingest_csv(
        ...
        *,
        ...
        records: list[Ticker] = []
        self._state.ingestor.historical_csv(
            path,
            records.append,
    ...
    def cache_frame(
        ...
        *,
        ...
        if frame.empty:
            ...
            normalized = self._state.integrity_validator.validate(
                normalized,
                layer=layer,
                symbol=symbol,
                venue=venue,
        ...
        cache = self._state.registry.cache_for(layer)
        cache.put(key, normalized)
        cached = cache.get(key)
        snapshot = self._build_snapshot(key, cached)
        self._state.metadata[key] = snapshot
        return cached
    ...
    def delete_cached_frame(
        ...
        *,
        ...
        metadata_removed = self._state.metadata.pop(key, None) is not None
        return removed_from_cache or metadata_removed

    def clear(self) -> None:
        """Reset the cache registry and forget all cached metadata."""

        self._state.registry = CacheRegistry()
        self._state.metadata.clear()


class DataIngestionQueryService(_DataIngestionBase):
    """Read-only analytics over cached ingestion datasets."""
...
class DataIngestionCacheService:
    """Facade that exposes dedicated command/query services for the cache."""
    ...
    def __init__(
        ...
        *,
        ...
        state = _CacheState(
            ingestor=data_ingestor or DataIngestor(),
            registry=cache_registry or CacheRegistry(),
            clock=clock or (lambda: datetime.now(UTC)),
            metadata={},
            integrity_validator=integrity_validator or TickFrameIntegrityValidator(),
        )
        self._state = state
        self._commands = DataIngestionCommandService(state)
        self._queries = DataIngestionQueryService(state)

    # ------------------------------------------------------------------
    # CQRS accessors
    @property
    def commands(self) -> DataIngestionCommandService:
        """Return the write-oriented service for cache mutations."""

        return self._commands

    @property
    def queries(self) -> DataIngestionQueryService:
        """Return the read-only service for analytics queries."""

        return self._queries

    # ------------------------------------------------------------------
    # Backwards compatible façade methods
    @property
    ...
    def ingest_csv(self, *args: object, **kwargs: object) -> pd.DataFrame:
        return self.commands.ingest_csv(*args, **kwargs)

    def cache_ticks(self, *args: object, **kwargs: object) -> pd.DataFrame:
        return self.commands.cache_ticks(*args, **kwargs)

    def cache_frame(self, *args: object, **kwargs: object) -> pd.DataFrame:
        return self.commands.cache_frame(*args, **kwargs)

    def delete_cached_frame(self, *args: object, **kwargs: object) -> bool:
        return self.commands.delete_cached_frame(*args, **kwargs)

    def get_cached_frame(self, *args: object, **kwargs: object) -> pd.DataFrame:
        return self.queries.get_cached_frame(*args, **kwargs)

    def metadata_for(self, *args: object, **kwargs: object) -> CacheEntrySnapshot | None:
        return self.queries.metadata_for(*args, **kwargs)

    def cache_snapshot(self) -> list[CacheEntrySnapshot]:
        return self.queries.cache_snapshot()
