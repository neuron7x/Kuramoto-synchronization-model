...
class KnowledgeUpdatePipeline:
    ...
    def _compress_segments(self, context: PipelineContext, segments: Sequence[DocumentSegment]) -> None:
        for compressed in self._compressor.bulk_compress(segments):
            context.compressed_segments[compressed.segment_id] = compressed

    def _index_segments(self, context: PipelineContext, segments: Sequence[DocumentSegment]) -> None:
