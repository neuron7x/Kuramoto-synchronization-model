...
2. **Phasic vs Tonic**:  phasic = ReLU(δ)·burst_factor   (positive part of δ)
...
class DopamineController:
    ...
    DopamineController v2.3 — апетитивний контур:
      ...
      • Фазика: phasic = max(0, RPE)·burst_factor.  # INV-DA1: phasic from positive RPE only
    ...
    def _load_and_validate_config(self, config_path: str) -> None:
        ...
        self.config["meta_adapt_rules"] = {
            state: dict(rules) for state, rules in self.config["meta_adapt_rules"].items()
        }
    ...
    def _init_cache_values(self) -> None:
        ...
        self._cache_logistic_clip_max: float = float(self.config.get("logistic_clip_max", 60.0))
        self._cache_logistic_clip_min: float = float(self.config.get("logistic_clip_min", -60.0))

    def _default_logger(self, name: str, value: float) -> None:
        try:
            from geosync.runtime.thermo_api import log_metric  # type: ignore
    ...
    def compute_rpe(
        ...
        gamma = self._cache_discount_gamma if discount_gamma is None else float(discount_gamma)
    ...
    def step(
        ...
        *,
        ...
        go_threshold, hold_threshold, no_go_threshold = self._get_gate_thresholds(ddm_info)
    ...
    def _get_gate_thresholds(self, ddm_info: Optional[DDMThresholds]) -> Tuple[float, float, float]:
    ...
    def compute_temperature(
        ...
        da = self._ensure_finite("dopamine_signal", da)
        base = (
            self._adaptive_base_temperature if base_temperature is None else float(base_temperature)
        )
    ...
    def check_invigoration(self, dopamine_signal: Optional[float] = None) -> bool:
        da = self.dopamine_level if dopamine_signal is None else float(dopamine_signal)
        da = min(1.0, max(0.0, da))  # INV-DA3: DA ∈ [0,1]
    ...
    def _clamp_meta_param(self, key: str, value: float) -> float:
        """Project a meta-adapted parameter onto its declared contract range.

        Bounds mirror ``_validate_core_params`` so ``meta_adapt`` can never emit a
        value the validator would later reject. ``base_temperature`` has no fixed
        upper bound but must stay > 0 and ≥ ``min_temperature``; the other two are
        hard closed-interval contracts.
        """
        if key == "learning_rate_v":
            return min(1.0, max(1e-9, value))  # bounds: learning_rate_v ∈ (0, 1]
        if key == "delta_gain":
            return min(1.0, max(0.0, value))  # bounds: delta_gain ∈ [0, 1]
        if key == "base_temperature":
            # bounds: base_temperature ≥ min_temperature (> 0)
            return max(float(self.config["min_temperature"]), value)
        return value
    ...
    def meta_adapt(self, performance_metrics: Mapping[str, float]) -> None:
        ...
        for key, factor in rules.items():
            ...
            drifted = float(old_value * factor)
            # Re-project the multiplicatively-drifted value back onto its declared
            # contract range. Without this, repeated meta-adaptation walks a
            # parameter geometrically out of bounds (learning_rate_v * 1.01^n > 1),
            # producing a config that _validate_core_params rejects on the next
            # reload — and which save_config_to_yaml would persist as invalid.
            # Clamp at the boundary; never let an out-of-contract value through.
            new_value = self._clamp_meta_param(key, drifted)
            if new_value != drifted:
                self._log(f"dopamine_meta_{key}_clamped", new_value - drifted)
            cfg[key] = new_value
            if key == "base_temperature":
    ...
    def save_config_to_yaml(self, path: Optional[str] = None) -> None:
        ...
        # Atomic write: serialise into a temp file in the same directory, fsync,
        # then os.replace onto the target. meta_adapt calls this from the control
        # loop, so a crash or a concurrent reader must never observe a truncated
        # half-written config (a plain open("w") leaves exactly that window).
        directory = os.path.dirname(os.path.abspath(target)) or "."
        fd, tmp_path = tempfile.mkstemp(dir=directory, prefix=".dopamine-", suffix=".yaml.tmp")
        try:
    ...
    def load_state(self, state: Mapping[str, float]) -> None:
        ...
        self._temp_adam_t = max(0, temp_adam_t)  # bounds: Adam step counter non-negative
        release_flag = self._ensure_finite("release_gate_open", float(state["release_gate_open"]))
