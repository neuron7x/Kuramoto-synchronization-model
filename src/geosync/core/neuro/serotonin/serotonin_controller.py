...
try:  # Allow loading as a standalone module in tests
    from geosync.core.neuro.serotonin.receptors import ParamDeltas, ReceptorBank, ReceptorContext
...
class SerotoninController:
    ...
    def _build_receptor_context(
        ...
        return ReceptorContext(
            ...
            # Feed the real timestep to the receptors' low-pass filters so their
            # time constants are step-size correct. Fall back to a deterministic
            # 1.0 when no dt is given (never wall-clock — that would make the
            # receptor states non-reproducible).
    ...
    def _apply_param_deltas(
        ...
        hysteresis_margin = float(
            ...
            )  # bounds: hysteresis_margin ∈ [0.01, 0.2]  (gate parameter; no 5HT invariant governs it)
        )
        ...
        )  # bounds: gate_veto ∈ [0, 1.2]  (veto-strength parameter; not the tonic integrator of INV-5HT6)
        phasic_veto = float(
            max(
                0.0, min(1.2, base["phasic_veto"] * (1.0 - 0.2 * max(veto_bias, 0.0)))
            )  # bounds: phasic_veto ∈ [0, 1.2]  (veto-strength parameter; not the aversive state of INV-5HT2)
        )
        ...
        serotonin_level = float(
            ...
            )  # INV-5HT1: serotonin level s ∈ [0, 1]
        )
    ...
    def step(
        ...
        with self._lock:
            ...
            if hold and not self._hold_state:
                ...
                self._cooldown = max(
                    0.0, cooldown_bias
                )  # bounds: cooldown ≥ 0  (cooldown timer parameter; not the sensitivity of INV-5HT4)
            ...
            if not self._hold_state and self._cooldown > 0:
                ...
                )  # bounds: cooldown ≥ 0  (cooldown timer decrement; not the sensitivity of INV-5HT4)
            self._hold = bool(self._hold_state)
    ...
    def _resolve_config_path(config_path: str) -> Path:
        ...
        if candidate.is_file():
            return candidate
        env_dir = os.getenv("GEOSYNC_CONFIG_DIR")
        if env_dir:
            for name in (candidate.name, "serotonin.yaml"):
                env_candidate = Path(env_dir) / name
                if env_candidate.is_file():
                    logging.getLogger(__name__).warning(
                        "Using serotonin config from GEOSYNC_CONFIG_DIR=%s", env_dir
                    )
    ...
    def compute_serotonin_signal(self, aversive_state: float) -> float:
        ...
        with self._lock:
            ...
            if self.tonic_level > cfg["cooldown_threshold"]:
                ...
                )  # INV-5HT4: sensitivity upper bound s ≤ 1.0
    ...
    def modulate_action_prob(
        ...
        with self._lock:
            ...
            )  # bounds: action probability ∈ [0, 1]  (not the temperature floor of INV-5HT5)
