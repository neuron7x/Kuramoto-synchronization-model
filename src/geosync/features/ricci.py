...
"""Ricci curvature graph adapter for GeoSync Neuro-Architecture.
...
class RicciCurvatureGraph:
    ...
    def _edge_curvature(
        ...
        # Each measure must sum to 1 on its OWN node: the neighbor mass is
        # therefore normalized by that node's own degree. Sharing u's mass with
        # μ_v mis-normalizes μ_v whenever deg(u) != deg(v) (μ_v would not sum to
        # 1), inflating the Wasserstein transport and biasing κ.
        mass_self = self.alpha
        mass_neighbor_u = (1.0 - self.alpha) / max(len(neighbors_u) - 1, 1)
        mass_neighbor_v = (1.0 - self.alpha) / max(len(neighbors_v) - 1, 1)
        ...
        for nu in neighbors_u:
            for nv in neighbors_v:
                ...
                if nu == u:
                    ...
                    mu_u = mass_neighbor_u

                if nv == v:
                    mu_v = mass_self
                else:
                    mu_v = mass_neighbor_v
