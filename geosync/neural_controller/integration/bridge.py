...
"""Neural controller integration bridge for GeoSync."""
...
class TACLSystem:
    ...
    def _resolve_impl() -> Any | None:
        provider = os.getenv("GEOSYNC_NEURO_TACL_PROVIDER", "noop").lower()
        ...
        if provider == "runtime":
            try:
                ...
                log.warning(
                    "Failed to load runtime ThermoController",
...
class KuramotoSync:
    ...
    def _resolve_monitor() -> Callable[[], float]:
        provider = os.getenv("GEOSYNC_NEURO_KURAMOTO_PROVIDER", "noop").lower()
