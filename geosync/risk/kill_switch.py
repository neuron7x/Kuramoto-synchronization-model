...
class KillSwitchCallbackError(RuntimeError):
    """Raised after a state change when one or more protective callbacks failed.

    A registered halt action (flatten/disconnect) that raises during a trip must
    NOT be silently swallowed: the switch state is already set, but the caller
    must learn that a protective side-effect did not complete. Per-callback
    isolation is preserved (one failure does not stop the others); the aggregate
    is surfaced fail-closed. Mirrors ``runtime.kill_switch.KillSwitchCallbackError``.
    """

    def __init__(self, is_active: bool, reason: str, errors: "list[BaseException]") -> None:
        super().__init__(
            f"{len(errors)} kill-switch callback(s) failed during "
            f"{'activation' if is_active else 'deactivation'} ({reason})"
        )
...
class SafetyController:
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            # Fail-CLOSED: a safety latch whose persisted state exists but is
            # unreadable must NOT silently re-arm trading on restart. If the
            # switch may have been tripped, come up HALTED and require an
            # explicit operator clear. (An ABSENT file is a clean first start,
            # handled by the early return above — only CORRUPTION lands here.)
            LOGGER.error(
                "Corrupt safety state — failing closed (HALTED); operator clear required",
                extra={
                    "event": "safety.load_failed_failclosed",
                    "error": str(exc),
                },
            )
            self._state.kill_switch_active = True
            self._state.kill_switch_reason = f"corrupt persisted state: {exc}"
            # No ambient wall-clock here (DETERMINISM_POLICY.md — the corrupt-load
            # fallback is not a trading event; forensic timing lives in the LOGGER.error
            # above, keeping the deterministic state free of ambient nondeterminism).
            self._state.kill_switch_timestamp = None
            self._state.mode = SafetyMode.HALTED
            self._state.force_paper_trading = True

    def _notify_callbacks(self) -> None:
        """Notify registered callbacks of state change — fail-closed on failure.

        Per-callback isolation is preserved, but failures are aggregated and
        surfaced: if ANY protective callback (flatten/disconnect) raised, a
        :class:`KillSwitchCallbackError` is raised after every callback has run.
        The switch state is already committed before this call, so trading stays
        halted; the raise ensures the caller cannot treat a trip as fully
        effected when a protective side-effect actually failed (was fail-open).
        """
        state = self.state
        ...
        for callback in self._callbacks:
            try:
                ...
                errors.append(exc)
        if errors:
            raise KillSwitchCallbackError(
                self._state.kill_switch_active, self._state.kill_switch_reason or "", errors
            )
