...
def var_es(returns: np.ndarray, alpha: float = 0.975) -> tuple[float, float]:
    ...
    if len(finite_returns) == 0:
        # Fail-closed: no finite data means tail risk is UNKNOWN, not zero.
        # Returning 0.0 would be a *legal* riskless ES that launders empty /
        # all-NaN input straight past check_risk_breach (a book of unknown risk
        # would read "OK"). NaN routes into check_risk_breach's non-finite path,
        # which fails closed to "BREACH".
...
def kelly_shrink(
    ...
    # Fail-closed: an unrecognized / None / empty / wrong-case EWS regime is
    # treated as maximally dangerous → size to zero, NOT half. A default of 0.5
    # here was fail-OPEN: a corrupted, new, or MORE-SEVERE early-warning state
    # (e.g. "UNKNOWN", "CATASTROPHE", wrong-case "kill", "", None) would launder
    # a non-flat HALF position past risk sizing. Exact-match contract only — we
    # do NOT case-fold safety-critical keys. INV-KELLY2: applied fraction ≤ cap;
    # unknown regime yields the safe floor.
    lambda_factor = lambda_map.get(ews_level, 0.0)
