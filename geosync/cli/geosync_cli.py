...
"""GeoSync CLI exposing ingest/backtest/optimize/exec/report workflows."""
...
from analytics.regime.src.core import geosync_v21 as v21
...
# execution.watchdog is an application-layer module; a static
# ``from execution.watchdog import Watchdog`` here is a layering inversion that
# the commit_acceptor_policy.yaml ``forbidden_import_patterns`` rule rejects in
# any touched file. This file becomes a touched file when re-homed from the
# repo-root ``cli/`` into the canonical ``geosync.cli`` namespace (#1158).
# Resolve the symbol through importlib at module load instead — identical object,
# zero static ``execution.*`` import node (the PR #551/#552
# grandfathered-forbidden-imports-fix pattern, mirrored from geosync.risk).
_exec_watchdog: Any = importlib.import_module("execution.watchdog")
Watchdog: Any = _exec_watchdog.Watchdog
...
def _load_prices(cfg: IngestConfig | BacktestConfig | ExecConfig) -> pd.DataFrame:
    ...
    if data_cfg.kind == "csv":
        ...
        try:
            ...
            raise ArtifactError(
                "Parquet sources require either pyarrow or polars. Install the 'geosync[feature_store]' extra."
            ) from exc
...
def _write_frame(frame: pd.DataFrame, destination: Path, *, command: str = "cli") -> str:
    ...
    if suffix in {".csv", ""}:
        ...
        try:
            ...
            raise ConfigError(
                "Writing parquet outputs requires either pyarrow or polars. Install the 'geosync[feature_store]' extra."
            ) from exc
    else:
...
Examples:
  ...
  geosync_cli ingest --config my_ingest.yaml
...
def completion(shell: str) -> None:
    ...
    This enables tab-completion for GeoSync CLI commands, options,
    ...
    Example:
      ...
      geosync_cli completion zsh

      # For fish (add to ~/.config/fish/config.fish)
      geosync_cli completion fish
    """

    prog_name = Path(sys.argv[0]).name or "geosync_cli"
...
def ingest(
    ...
    Examples:
      ...
      geosync_cli ingest --generate-config --template-output my_ingest.yaml
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                ...
                "Example: geosync_cli ingest --generate-config --template-output ingest.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli ingest --generate-config --template-output ingest.yaml"
        )
...
def backtest(
    ...
    Examples:
      # Generate a starter configuration
      geosync_cli backtest --generate-config --template-output backtest.yaml

      # Run backtest and display results as a table
      geosync_cli backtest --config backtest.yaml --output table

      # Run backtest with JSON Lines output (for pipelines)
      geosync_cli backtest --config backtest.yaml --output jsonl
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                "--template-output is required when using --generate-config.\n"
                "Example: geosync_cli backtest --generate-config --template-output backtest.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli backtest --generate-config --template-output backtest.yaml"
        )
...
def optimize(
    ...
    Examples:
      # Generate a starter configuration
      geosync_cli optimize --generate-config --template-output optimize.yaml

      # Run optimization with table output
      geosync_cli optimize --config optimize.yaml --output table
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                "--template-output is required when using --generate-config.\n"
                "Example: geosync_cli optimize --generate-config --template-output optimize.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli optimize --generate-config --template-output optimize.yaml"
        )
...
def exec(
    ...
    Examples:
      # Generate a starter configuration
      geosync_cli exec --generate-config --template-output exec.yaml

      # Run signal evaluation
      geosync_cli exec --config exec.yaml

      # Run with human-readable table output
      geosync_cli exec --config exec.yaml --output table

    \b
    Tip: Use the 'serve' alias for the same functionality:
      geosync_cli serve --config exec.yaml
    """
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                "--template-output is required when using --generate-config.\n"
                "Example: geosync_cli exec --generate-config --template-output exec.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli exec --generate-config --template-output exec.yaml"
        )
...
def report(
    ...
    Examples:
      # Generate a starter configuration
      geosync_cli report --generate-config --template-output report.yaml

      # Generate a report from multiple inputs
      geosync_cli report --config report.yaml

      # Show report metadata as a table
      geosync_cli report --config report.yaml --output table
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                "--template-output is required when using --generate-config.\n"
                "Example: geosync_cli report --generate-config --template-output report.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli report --generate-config --template-output report.yaml"
        )
...
def deploy(
    ...
    """Apply GeoSync Kubernetes manifests via kubectl.

    Deploys GeoSync to a Kubernetes cluster using Kustomize overlays.
    Supports dry-run mode, rollout waiting, and deployment annotations.

    \b
    Examples:
      # Generate a starter deployment config
      geosync_cli deploy --generate-config --template-output deploy.yaml

      # Deploy to staging environment
      geosync_cli deploy --config deploy.yaml
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                "--template-output is required when using --generate-config.\n"
                "Example: geosync_cli deploy --generate-config --template-output deploy.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli deploy --generate-config --template-output deploy.yaml"
        )
    ...
    annotations = {
        "geosync.dev/artifact-digest": cfg.artifact,
        "geosync.dev/strategy-id": cfg.strategy,
    }
...
def parity(
    ...
    Examples:
      # Generate a starter configuration
      geosync_cli parity --generate-config --template-output parity.yaml

      # Synchronize features with the online store
      geosync_cli parity --config parity.yaml
    ...
    if generate_config:
        if template_output is None:
            raise click.UsageError(
                "--template-output is required when using --generate-config.\n"
                "Example: geosync_cli parity --generate-config --template-output parity.yaml"
            )
    ...
    if config is None:
        raise click.UsageError(
            ...
            "To generate a starter config: geosync_cli parity --generate-config --template-output parity.yaml"
        )
...
def fete_backtest(
    ...
    Examples:
      ...
      geosync_cli fete-backtest --csv data/prices.csv

      # Custom column names and output
      geosync_cli fete-backtest --csv data.csv --price-col close --out equity.csv

      # With model probabilities
      geosync_cli fete-backtest --csv data.csv --prob-col signal_prob
...
def causal_pipeline(
    ...
    Runs the GeoSync V21 causal inference pipeline for market regime
    ...
    Examples:
      ...
      geosync_cli causal-pipeline --returns-csv returns.csv --output results.json

      # Run from precomputed features
      geosync_cli causal-pipeline --features-csv features.csv

      # Custom window and horizon
      geosync_cli causal-pipeline --returns-csv data.csv --window 126 --horizon 10
    ...
    if bool(returns_csv) == bool(features_csv):
        raise click.UsageError(
            ...
            "Example with returns: geosync_cli causal-pipeline --returns-csv returns.csv\n"
            "Example with features: geosync_cli causal-pipeline --features-csv features.csv"
        )
    ...
    pipeline = v21.GeoSyncV21Pipeline(
        feature_builder,
        trainer,
        hmm,
        backtester,
...
# GeoSync documentation, however, now standardises on the verbs ``train`` and
